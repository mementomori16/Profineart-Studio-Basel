import * as dotenv from 'dotenv';
import express, { Request, Response } from 'express';
import cors from 'cors';
import helmet from 'helmet';
import { onRequest } from "firebase-functions/v2/https"; // Firebase V2 Trigger
import { getAvailableSlots } from './services/availabilityService.js';
import { createStripeCheckoutSession, fulfillOrder } from './services/checkoutService.js';
import { validateAddress } from './services/geocodingService.js';
import Stripe from 'stripe';

dotenv.config();

// Fix for Type Error 2322 (Stripe Version Mismatch)
const STRIPE_API_VERSION = '2025-12-15.clover' as any;

// Initialize Stripe
const stripe = new Stripe(process.env.STRIPE_SECRET_KEY as string, {
    apiVersion: STRIPE_API_VERSION,
});

const app = express();

// --- Security Middleware Stack ---
app.use(helmet()); 

// CORS: Updated to allow your live production domain
app.use(cors({
    origin: [
        process.env.CLIENT_URL || 'http://localhost:5173',
        'https://profineart.ch', 
        /\.web\.app$/,           
        /\.firebaseapp\.com$/ 
    ],
    methods: ['GET', 'POST'],
    credentials: true,
}));

app.use(express.json());

// --- Helper Function ---
function getErrorMessage(error: unknown): string {
    if (error instanceof Error) return error.message;
    return String(error);
}

// --- Routes ---

app.get('/', (req: Request, res: Response) => {
    res.json({ message: 'Server is running via Firebase Functions.' });
});

app.get('/api/schedule/slots', async (req: Request, res: Response) => {
    try {
        const { productId, date, duration } = req.query;
        if (!productId || typeof productId !== 'string' || !date || typeof date !== 'string' || !duration) {
            return res.status(400).json({ success: false, message: 'Missing parameters' });
        }

        const durationMinutes = parseInt(duration as string, 10);
        const slots = await getAvailableSlots(productId, date, durationMinutes);
        return res.json({ success: true, slots }); // Added return
    } catch (error) {
        console.error('API Error:', getErrorMessage(error));
        return res.status(500).json({ success: false, message: 'Server error fetching slots.' }); // Added return
    }
});

app.post('/api/validate-address', async (req: Request, res: Response) => {
    try {
        const { address } = req.body;
        if (!address) {
            return res.status(400).json({ success: false, message: 'Address required.' }); // Added return
        }

        const validationResult = await validateAddress(address);
        return res.status(validationResult.isValid ? 200 : 422).json({ 
            success: validationResult.isValid, 
            message: validationResult.message 
        }); // Added return
    } catch (error) {
        return res.status(500).json({ success: false, message: 'Address validation failed.' }); // Added return
    }
});

app.post('/api/create-checkout-session', async (req: Request, res: Response) => {
    try {
        if (!req.body.packageId || !req.body.email) {
            return res.status(400).json({ success: false, message: 'Missing info.' }); // Added return
        }
        const session = await createStripeCheckoutSession(req.body, stripe);
        return res.json({ checkoutUrl: session.url }); // Added return
    } catch (error) {
        return res.status(500).json({ success: false, message: 'Stripe error.' }); // Added return
    }
});

app.post('/api/order/fulfill', async (req: Request, res: Response) => {
    try {
        const { sessionId } = req.body;
        if (!sessionId) {
            return res.status(400).json({ success: false, message: 'Invalid session ID.' }); // Added return
        }

        const fulfillmentResult = await fulfillOrder(sessionId, stripe);
        return res.json({ success: true, result: fulfillmentResult }); // Added return
    } catch (error) {
        return res.status(500).json({ success: false, message: 'Fulfillment failed.' }); // Added return
    }
});

// --- EXPORT FOR FIREBASE ---
export const api = onRequest({ region: "europe-west1", memory: "256MiB" }, app);
